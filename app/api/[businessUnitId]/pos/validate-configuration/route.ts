...
async function validatePosConfiguration(businessUnitId: string) {
  ...
  if (menuItemsWithoutMapping > 0) {
    if (posConfig.autoPostToGl) {
      issues.push(`${menuItemsWithoutMapping} menu items are missing GL account mappings`)
    } else {
      warnings.push(`${menuItemsWithoutMapping} menu items are missing GL account mappings`)
    }
  }
  ...
  if (paymentMethodsWithoutMapping > 0) {
    if (posConfig.autoPostToGl) {
      issues.push(`${paymentMethodsWithoutMapping} payment methods are missing GL account mappings`)
    } else {
      warnings.push(`${paymentMethodsWithoutMapping} payment methods are missing GL account mappings`)
    }
  }
