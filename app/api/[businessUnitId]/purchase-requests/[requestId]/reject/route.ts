...
import { Prisma, PurchaseOrderItem, DocumentStatus, PurchaseRequestStatus } from "@prisma/client"
...
export async function PATCH(
  ...
  try {
    ...
    // Check if request exists and can be rejected
    ...
    const purchaseRequest = await prisma.purchaseRequest.update({
      ...
      data: {
        ...
        approvalDate: new Date() // Corrected from approvedAt
      },
      include: {
        requestor: {
          select: {
    ...
    console.error("[PURCHASE_REQUEST_REJECT]", error)
    return new NextResponse("Internal error", { status: 500 })
  }
}
