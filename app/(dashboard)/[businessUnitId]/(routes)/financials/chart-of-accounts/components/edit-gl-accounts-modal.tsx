...
const editGlAccountSchema = z.object({
...
export const EditGlAccountModal = ({
  isOpen,
  onClose,
  onSuccess,
  account
  ...
  const form = useForm<z.infer<typeof editGlAccountSchema>>({
    resolver: zodResolver(editGlAccountSchema),
    defaultValues: {
      accountCode: "",
      name: "",
      accountTypeId: "",
  ...
  const onSubmit = async (values: z.infer<typeof editGlAccountSchema>) => {
    if (!account) return

    try {
      setLoading(true)
      
      await axios.patch(`/api/${businessUnitId}/gl-accounts/${account.id}`, values, {
        headers: {
      ...
      toast.error(`Failed to update GL account: ${error}`)
    } finally {
      setLoading(false)
    }
  }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the general ledger account information.
          </DialogDescription>
        </DialogHeader>
