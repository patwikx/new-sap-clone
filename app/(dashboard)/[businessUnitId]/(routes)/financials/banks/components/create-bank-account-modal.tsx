...
export const CreateBankAccountModal = ({
  ...
  const onSubmit = async (values: z.infer<typeof createBankAccountSchema>) => {
    try {
      setLoading(true)
      
      await axios.post(`/api/${businessUnitId}/bank-accounts`, values, {
        headers: {
          'x-business-unit-id': businessUnitId
        }
      })
      ...
      toast.error(`Failed to create bank account: ${error}`)
    } finally {
      setLoading(false)
    }
  }
